...
required_functions:
  ...
  - snapshot_functions.sdg.get_table
common_outputs:
  - notebook
  - csv
pitfalls:
  - only ~22-25% of ANI papers are SDG-classified — left join only
  - use with_labels=True for human-readable SDG names
review_checks:
  - verify left join to SDG
  - verify nopp() on ANI
  - verify with_labels=True if label column is used
---
