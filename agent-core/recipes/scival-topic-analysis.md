...
required_functions:
  ...
  - column_functions.long_eid_to_eidstr
  - dataframe_functions.df_cached
  - snapshot_functions.scival.get_table
common_outputs:
  - notebook
  - csv
pitfalls:
  - must convert ANI.Eid (long) to EidString with long_eid_to_eidstr() before joining
  - not all ANI EIDs appear in topic_eid — left join only
  - topic cluster is the broader grouping; topic is the fine-grained level
review_checks:
  - verify long_eid_to_eidstr conversion present
  - verify left join to topic_eid
  - verify countDistinct(Eid) not double-counting from exploded joins
---
