...
common_outputs:
  ...
  - csv
pitfalls:
  - ADS is broader than nopp()-filtered ANI — left join only
  - choose correct FWCI variant (4y/5y/no-window; all/no-self-cit)
review_checks:
  - verify left join to ADS
  - verify nopp() on ANI
  - verify correct FWCI column name from ADS reference
---
