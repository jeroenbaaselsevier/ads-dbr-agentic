...
# MAGIC %md
# MAGIC # Burst Score — Topic Clusters
# MAGIC
# MAGIC Calculates annual burst scores for SciVal **topic clusters** based on a 6-year
...
# MAGIC
# MAGIC Output: `/mnt/els/rads-mappings/burst_analysis/clusters/<analyze_year>/`
...
# Output path
path_output = f'/mnt/els/rads-mappings/burst_analysis/clusters/{analyze_year}/'
...
# MAGIC %md
# MAGIC # Load SciVal topic → topic-cluster mapping

# COMMAND ----------

# Join topic_eid (Eid ↔ TopicId) with topic_topiccluster (TopicId ↔ TopicClusterId)
# to get a direct Eid ↔ Topic_Cluster mapping
df_topic = (
    ...
    .join(
        sf.scival.get_table('topic_topiccluster', snapshot=scopus_snapshot_date)
          .select('TopicId', F.col('TopicClusterId').alias('Topic_Cluster')),
        'TopicId',
    )
    .select('Topic_Cluster', 'Eid')
    .distinct()
)
...
# MAGIC %md
# MAGIC # Compute per-cluster metrics
...
# Year scaffold — ensures every cluster gets a row even for years with no data
...
df_views = (
    ...
    .join(df_topic, 'Eid', 'left')
    .select('Topic_Cluster', 'data_year', 'Eid', 'abstract_views', 'outward_links', 'sort_year')
...
df_cluster_info = (
    df_years
    .crossJoin(df_topic.select('Topic_Cluster').distinct())
    .join(df_pubs,      ['Topic_Cluster', 'data_year'], 'left')
    .join(df_views,     ['Topic_Cluster', 'data_year'], 'left')
    .join(df_citations, ['Topic_Cluster', 'data_year'], 'left')
    .join(df_citescore, ['Topic_Cluster', 'data_year'], 'left')
...
df_cluster_prom = (
    ...
    .withColumn('Rank', F.rank().over(
        Window.partitionBy('data_year')
              .orderBy(F.desc('Prominence'), F.desc('output'), F.asc('Topic_Cluster'))
    ))
...
display(spark.read.parquet(path_output).filter('Topic_Cluster = 918'))
