...
# 7b:
#  * allow use of a minyear_overall
# 7c: 
#  * set maxyear also to None
#  * being able to include/exclude pp
#  * minyear_overall optional (can be None)
# 7d:
#  * variable threshold for the minimum number of arcpre in career (was hardcoded 5)
#  * allow mincityear < minyear_overall but cast warning
#  * allow min_npY1Y3 filter
#  * ignore citations TO non-medl/scopus content (very rare - should not happen anyway)
# 7e:
#  * set a maximum CITATION year
# 7f:
#  * allow limitation of cited doc types
# 7g:
#  * ignore nulls in country selection to better catch the country for an affiliation
# 8:
#  * move coding back to standard ADS (remove ICSR LAB specific coding)
# 9:
#  * add counts of Retraction status papers from RWDB
#  * remove pubs/cit discontinued titles.
# 10:
#  * run rwdb functions from library
#  * use apr table as preferred entry for name / institution
...
try:
  ...
  dbutils.notebook.exit(f"this notebook should not be run directly. Missing parameter: {str(e)}")
# set defaults
try:
  minyear_overall
except NameError:
  minyear_overall=None
print('use minyear_overall '+str(minyear_overall))

try:
  maxcityear
except NameError:
  maxcityear=None
print('use maxcityear '+str(maxcityear))
...
try:
  ...
  includepp=False
print('use includepp '+str(includepp))
  
try:
  min_count_arcpre
except NameError:
  min_count_arcpre=5
print('use min_count_arcpre '+str(min_count_arcpre))
  
try:
  min_npY1Y3
except NameError:
  min_npY1Y3=2
print('use min_npY1Y3 '+str(min_npY1Y3)+' (only affects post-agg tables)')
...
if minyear_overall is not None:
  if minyear_overall > mincityear:
    print("Warning: minyear_overall is larger than mincityear. This means citations from <"+str(minyear_overall)+" will be included ")


doctypes_filename_add=''
print('including preprints in np count and field designation' if includepp else 'excluding preprints')
if doctypes is None:
  print('use no doctypes filter')
else:
  print('Limit to doctypes for count based indicators: '+",".join(doctypes))
  print('this filter does not affect first/last pub, institution, field assignment etc')
  doctypes_filename_add='_doctypes_'+''.join(doctypes)
  if includepp and not 'pp' in doctypes:
    print('doctypes filter will override the pp setting and exclude preprints in np counts')

csvOutFileName_s1=basePath_project+'Table-S1_'+ani_stamp+'_'+minyear+'_'+mincityear+'_'+maxyear+'_'+Y4+'_pp'+str(includepp)+'_minarcpre_'+str(min_count_arcpre)+'_minnpY1Y3_'+str(min_npY1Y3)+'_maxcityear_'+str(maxcityear)+doctypes_filename_add
csvOutFileName_s3=basePath_project+'Table-S3_'+ani_stamp+'_'+minyear+'_'+mincityear+'_'+maxyear+'_'+Y4+'_pp'+str(includepp)+'_minarcpre_'+str(min_count_arcpre)+'_minnpY1Y3_'+str(min_npY1Y3)+'_maxcityear_'+str(maxcityear)+doctypes_filename_add
...
import json
rw_storage_folder=basePath_project+"cache/"+ani_stamp
rw_storage_json=os.path.join(rw_storage_folder,"rwdb.json")
# cache the RWDB snapshot
if not file_exists(rw_storage_json):
    os.makedirs(os.path.dirname(f"/dbfs{rw_storage_json.replace('dbfs:/','/')}"),exist_ok=True)
    with open(f"/dbfs{rw_storage_json.replace('dbfs:/','/')}", 'w') as file:
        json.dump({'rwdb_csv':check_and_fetch_rwdb_csv(rw_storage_folder)}, file)   
with open(f"/dbfs{rw_storage_json.replace('dbfs:/','/')}", 'r') as file:
    path_rw_dump_csv = json.load(file)['rwdb_csv']


# COMMAND ----------

df_ani_rw=get_clean_rw_df(path_rw_dump_csv, ani_stamp, rw_storage_folder)
...
## TODO: take a curated name from SCOPUS APR as LEADING over a latest name in ani(?)

tbn_df_ani_auth_name="temp_df_ani_auth_name"
savepath_auth_name=basePath_project+"cache/"+ani_stamp+"/"+tbn_df_ani_auth_name
if file_exists(savepath_auth_name):
  ...
  df_ani_auth_name_t=(
    ...
    .select(
      'auid',
      func.col('Eid').alias('LastEidName'),
      #func.coalesce(func.expr('CONCAT(Au.surname_pn,", ",IFNULL(Au.given_name_pn,Au.initials_pn))'),func.col('Au.indexed_name_pn'),func.col('Au.indexed_name')).alias('authfull'),
      func.coalesce(
        func.concat(
            func.col('Au.surname_pn'),
            func.lit(', '),
            func.when(
                func.col('Au.given_name_pn').isNotNull(),
                func.col('Au.given_name_pn')
            ).otherwise(func.col('Au.initials_pn'))
        ),
        func.col('Au.indexed_name_pn'),
        func.col('Au.indexed_name')
      ).alias('authfull'),
      'lastPubYear','firstPubYear'
    )
  )
...
from pyspark.sql import functions as func
# table with name/inst from apr. we will use this as the source of truth for author names and institutions, and will only collect names from the other tables if null/not present in the table
tbn_df_apr_auth_name_inst="temp_df_apr_auth_name_inst"
savepath_apr_auth_name_inst=basePath_project+"cache/"+ani_stamp+"/"+tbn_df_apr_auth_name_inst
if file_exists(savepath_apr_auth_name_inst):
  print("path already exists: "+savepath_apr_auth_name_inst)
else:
  print ("Generating "+savepath_apr_auth_name_inst)
  (
    df_apr
    .select(
        'auid',
        # func.array_sort(
        #     func.col('affiliation_current_full'),
        #     lambda left, right: func.when(
        #         left["type_afid"] == "parent", -1)
        #         .when(right["type_afid"] == "parent", 1)
        #         .otherwise(0)
        # )[0].alias('affiliation_current_full'),
        # Sorts the 'affiliation_current_full' array for each author, prioritizing 'parent' type affiliations,
        # and otherwise preserving the original order. Returns the first (top-ranked) affiliation.
        # This is used to select the most relevant institution for each author.
        func.array_sort(
            func.transform(
                func.col("affiliation_current_full"),
                lambda x, i: func.struct(
                    x["type_afid"].alias("type_afid"),
                    x["afdispname"].alias("afdispname"),
                    x["afid"].alias("afid"),
                    
                    x["country_tag"].alias("country_tag"),
                    func.lit(i).alias("orig_idx")
                )
            ),
            lambda left, right: func.when(
                (left["type_afid"] == right["type_afid"]),
                left["orig_idx"] - right["orig_idx"]
            ).when(
                left["type_afid"] == "parent", -1
            ).when(
                right["type_afid"] == "parent", 1
            ).otherwise(left["orig_idx"] - right["orig_idx"])
        )[0].alias("affiliation_current_full"),
        func.coalesce(
            func.concat(
                func.col('surname_pn'),
                func.lit(', '),
                func.when(
                    func.col('given_name_pn').isNotNull(),
                    func.col('given_name_pn')
                ).otherwise(func.col('initials_pn'))
            ),
            func.col('indexed_name_pn')
        ).alias('apr_authfull'),
    )
    .withColumn('apr_affil_id',func.col('affiliation_current_full.afid'))
    .withColumn('apr_inst_name',func.col('affiliation_current_full.afdispname'))
    .withColumn('apr_cntry',func.col('affiliation_current_full.country_tag'))
    .drop('affiliation_current_full')
  ).repartition(200).write.format("parquet").save(savepath_apr_auth_name_inst)
...
def get_df_mincityear_onw_cit(df_ani):
  return (
    ...
    .filter(func.lit(True) if maxcityear is None else func.col(sort_pub_year)<=maxcityear)
    ...
    .withColumn('references_u',func.array_distinct('citations'))
    .select(
    ...
    .join(df_ani_rw.withColumnRenamed('eid','CitingEid').withColumnRenamed('isRetracted','isRetractedCiting'),'CitingEid','LEFT_OUTER') # citation from retraction
    .join(
      df_ani
      # ignore citations TO non SCOPUS/MEDL content.
      .filter(func.arrays_overlap('dbcollections',func.array(func.lit('SCOPUS'),func.lit('MEDL'))))
      .select(
        'Eid',
    ...
    .select(
      "CitingEid",
      "Eid",
      'isDiscontinuedCiting','isRetractedCiting',
    ...
    .agg(
      ...
      func.count("isRetractedCiting").alias('CitationCountFromRetracted')
...
# MAGIC %md # citations - per cited/citing author
# MAGIC Currently not used. Idea is to calculate the per citing author the unique citations they contribute.

# COMMAND ----------

# citation counts (mincityear+)
# per cited author - citing author - [citing docs]
# then do a cumulative collect set over those.
def get_df_mincityear_onw_cit_author(df_ani):
  return (
    df_ani
    .filter(sort_pub_year+' >= '+mincityear)
    # below filter only works from ICSR-Lab-data-v005+ is needed to exclude pre-print citations.
    ...
    .select(
      func.col('Eid').alias('CitingEid'),
      func.explode('references_u').alias('Eid'),
      # remove empty/null authors so we can filter later
      func.array_except(func.array_distinct(func.col('Au.auid').cast('array<long>')),func.array(func.lit(None),func.lit(0))).alias('CitingAuids')
    )
    # we only want to count citations where we have authors in this case, so we can reletivize the shares of citing authors to citing papers with authors.
    .filter('size(CitingAuids)>0')
    .join(
      df_ani
      #TODO: check if we need to filter here the cited pool of articles too.
      .select(
        'Eid',
        func.array_distinct(func.col('Au.auid').cast('array<long>')).alias('CitedAuids')
      ),["Eid"]
    )
    .withColumn('overLappingAuthors',func.size(func.array_intersect('CitingAuids','CitedAuids')))
    .filter('overLappingAuthors=0') # no self-citations
    .select('CitingEid','Eid',func.explode('CitedAuids').alias('auid'),'CitingAuids')
    .withColumn('TotalNonNullAuidCitations',func.count('*').over(Window.partitionBy('auid')))
    .withColumn('citing_auid',func.explode('CitingAuids'))
    .groupBy('auid','citing_auid')
    .agg(
      func.collect_set(func.struct('Eid','CitingEid')).alias('citation_pairs')       
    )
    .withColumn(
      'cumulative_citations',
      func.size(
        func.array_distinct(func.flatten(func.collect_set("citation_pairs").over(
          Window
          .partitionBy(func.col("auid"))
          .orderBy(func.desc(func.size('citation_pairs')))
          .rowsBetween(Window.unboundedPreceding, Window.currentRow)
        )))
      ).alias('cumulative_citations')
    )
    # now add a dense rank for the cumulative citations.
    # dense, so that ties do not occupate any space and it serves as a count for the number of authors needed
    .withColumn(
      'number_of_authors_needed_for_cumulative_citations',
      func
      .dense_rank()
      .over(
        Winddow.partitionBy('auid').orderBy(func.asc('cumulative_citations'))
      )
    )
  )
...
savepath_agg_count=basePath_project+"cache/"+ani_stamp+"/"+tbn_df_agg_count+'_'+minyear+'_'+mincityear+'_'+maxyear+'_'+Y4+'_pp'+str(includepp)+'_minarcpre_'+str(min_count_arcpre)+'_maxcityear_'+str(maxcityear)+doctypes_filename_add
if file_exists(savepath_agg_count):
  ...
  df_agg_count_t=(
    ...
    .na.fill({'CitationCountNonSelf':0,'CitationCount':0,'CitationCountFromDiscontinuedSources':0,'CitationCountFromRetracted':0})
    .select(
      'Eid',
      sort_pub_year,
      ...
      'CitationCountNonSelf','CitationCount','CitingEidsNonSelf','CitingEids',func.col('source.srcid').alias('srcid'),'citation_type','CitationCountFromDiscontinuedSources','CitationCountFromRetracted'
    )
    .withColumn('auid',func.col('Au.auid').cast('long'))
    .withColumn('Authorseq',func.col('Au.Authorseq')).drop('Au')
    .filter('auid > 100 AND auid IS NOT NULL')
    
    # join ScienceMetrix to count volume per subfield code
    .join(df_smc,["Eid"],"LEFT_OUTER")
    .withColumn('subfield',func.expr('IF(citation_type IN ("ar","cp","re"),subfield,NULL)')) # assign field only if ar/cp/re
    .withColumn('totalDocCountAuthor_alltime',func.count('*').over(w_au))
    .withColumn('arcpreDocCountAuthor_alltime',func.count(func.expr('IF(citation_type IN ("ar","cp","re"),Eid,NULL)')).over(w_au))
    .filter('arcpreDocCountAuthor_alltime>='+str(min_count_arcpre)) # only authors with >= 5 ar/cp/re's 
    ...
    # do this filter BEFORE ranks for h/hm
    ...
    # limit to doctypes based on provided filter
    .filter(func.lit(True) if doctypes is None else func.col('citation_type').isin(doctypes))
    ...
    .agg(
      ...
      func.count(func.expr('IF('+sort_pub_year+' BETWEEN '+minyear+' AND '+maxyear+',isRetracted,NULL)')).alias('npY1Y3_rw'),
      func.sum(func.when(func.col('isRetracted').isNull(),func.lit(0)).otherwise(func.col('CitationCount'))).alias('ws_ncY2Y3_to_rw'),
      func.count('isRetracted').alias('np_rw'),
      func.sum('CitationCountFromRetracted').alias('ws_ncY2Y3_rw'),
...
savepath_agg_count_ln_max=basePath_project+"cache/"+ani_stamp+"/"+tbn_df_agg_count_ln_max+'_'+minyear+'_'+mincityear+'_'+maxyear+'_'+Y4+'_pp'+str(includepp)+'_minarcpre_'+str(min_count_arcpre)+'_maxcityear_'+str(maxcityear)+doctypes_filename_add
...
savepath_agg_count_ln_wmaxc=basePath_project+"cache/"+ani_stamp+"/"+tbn_df_agg_count_ln_wmaxc+'_'+minyear+'_'+mincityear+'_'+maxyear+'_'+Y4+'_pp'+str(includepp)+'_minarcpre_'+str(min_count_arcpre)+'_maxcityear_'+str(maxcityear)+doctypes_filename_add

def get_lnmaxvalues_csv():
  df_agg_count_ln_max=spark.read.format("parquet").load(savepath_agg_count_ln_max)
  if not file_exists(savepath_agg_count_ln_wmaxc+'_lnmaxvalues.csv'):
    df_agg_count_ln_max.repartition(1).write.mode("overwrite").csv(savepath_agg_count_ln_wmaxc+'_lnmaxvalues.csv',header = 'true')
  return spark.read.format('csv').option('header',True).load(savepath_agg_count_ln_wmaxc+'_lnmaxvalues.csv')
...
savepath_auth_affinst_nonnull=basePath_project+"cache/"+ani_stamp+"/"+tbn_df_ani_auth_affinst_nonnull
if file_exists(savepath_auth_affinst_nonnull):
  ...
  df_ani_auth_affinst_nonnull_t=(
    ...
    # using full list of affliation_ids now, and de-duplicating.
    .withColumn('affil_id',func.explode('affiliation.affiliation_ids'))
    .groupBy('eid','affil_id','auid')
    .agg(
      func.first('affiliation').alias('affiliation'),
      func.first('datesort').alias('datesort')
    )
    .groupBy('auid','affil_id')

    .agg(
      func.count('*').alias('aff_occurences'),
      func.first(func.array_join('affiliation.affiliation_organization',", "),True).alias('affil_name'),
      func.first('affiliation.affiliation_tag_country',True).alias('cntry'),
...
if file_exists(csvOutFileName_s1+'.parquet'):
  ...
  df_apr_auth_name_inst=spark.read.format("parquet").load(savepath_apr_auth_name_inst)
  ...
  (
    ...
    .filter("npY1Y3 >= "+str(min_npY1Y3)) # only rank authors >= 2 papers since 1960
    .join(spark.read.format("parquet").load(savepath_auth_name),["auid"],"LEFT_OUTER")
    .join(df_ani_auth_affinst,["auid"],"LEFT_OUTER")

    # prefer apr names
    .join(df_apr_auth_name_inst,'auid','LEFT_OUTER')
    .withColumn('authfull',func.coalesce('apr_authfull','authfull'))
    .withColumn('cntry',func.coalesce('apr_cntry','cntry'))
    .withColumn('inst_name',func.coalesce('apr_inst_name','inst_name'))
    .withColumn('affil_id',func.coalesce('apr_affil_id','affil_id'))
...
print(ani_stamp+" has "+str(df_agg_result_parquet.count())+" authors")
print(ani_stamp+" has "+str(df_agg_result_parquet.filter('sm_field IS NOT NULL').count())+" authors with field assigned")# 
...
def authSubjectWindowColumns(window_partition,window_rank_fields,df,tableTag,column_prefix=''):
  cacheLoc=basePath_project+"cache/"+ani_stamp+"/"+"TS3_window_"+tableTag+"_"+column_prefix+"_"+minyear+'_'+mincityear+'_'+maxyear+'_'+Y4+'_pp'+str(includepp)+'_minarcpre_'+str(min_count_arcpre)+'_maxcityear_'+str(maxcityear)+doctypes_filename_add+'_minnpY1Y3_'+str(min_npY1Y3)
  if file_exists(cacheLoc):
    ...
    ).write.format('parquet').save(cacheLoc)
  return spark.read.format('parquet').load(cacheLoc)
...
def windowedTableS3(df,window_partition,tableTag):
  cacheLoc=basePath_project+"cache/"+ani_stamp+"/"+"TS3_grouped_"+tableTag+"_"+minyear+'_'+mincityear+'_'+maxyear+'_'+Y4+'_pp'+str(includepp)+'_minarcpre_'+str(min_count_arcpre)+'_maxcityear_'+str(maxcityear)+doctypes_filename_add+'_minnpY1Y3_'+str(min_npY1Y3)
  if file_exists(cacheLoc):
    print("skip writing "+cacheLoc)
  else:
    print("writing "+cacheLoc) 
...
def select_fields_top_list_output(df):
  return (
    ...
    .select(
      ...
      func.col("npY1Y3_rw").alias('np'+Y1+Y3+'_rw'),
      func.col("ws_ncY2Y3_to_rw").alias('nc'+Y2+Y3+'_to_rw'),
      func.col("ws_ncY2Y3_rw").alias('nc'+Y2+Y3+'_rw'),
...
[f'np{Y1}{Y3}_rw','',f'# papers {minyear}-{maxyear} marked as Retraction in RWDB'],
[f'nc{Y2}{Y3}_to_rw','',f'total cites {mincityear}-{maxyear} to papers (by this author) marked as Retraction in RWDB'],
[f'nc{Y2}{Y3}_rw','',f'total cites {mincityear}-{maxyear} from papers (by any author) marked as Retraction in RWDB'],
...
def save_wb(wb,fn_base):
  ...
  s3target=os.path.join(basePath_project,'excel_export',export_filename_pattern,f'{fn_base}.xlsx')
  dbutils.fs.cp(f'dbfs:/FileStore/{fn_base}.xlsx',s3target)
  print(f'stored for project: {s3target}')
           
  return f'/files/{fn_base}.xlsx'  
...
def gen_workbook_table1(fn_base,df_a,key_data):
  ...
  wsl = wsl_fill_sheet_data(df_a.drop('author_id').select(*[x[0] for x in key_data]),wb.create_sheet("Data"))
