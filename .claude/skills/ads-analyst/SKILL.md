...
2. **Plan** — use todo list
3. **Schema lookup** — `agent-core/catalog/knowledge-index.yaml` → relevant `agent-core/references/*.md`
4. **Recipe check** — `agent-core/recipes/` for common patterns
5. **Generate notebook** — mandatory boilerplate from `agent-core/roles/notebook-implementer.md`
6. **Deploy** — `./deploy.sh notebooks/<name>.py --run`
7. **Monitor** — `./poll_run.sh <run_id>`
8. **Decode results** — `agent-core/runbooks/databricks.md`
9. **Local post-processing** — `agent-core/runbooks/local-python.md`
10. **Review** — run `agent-core/roles/reviewer.md` checklist
